    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form action="{{ route('brands.update', $brand) }}" method="POST" id="form-edit-{{ $brand->id }}" onsubmit="confirmarActualizacion(event, {{ $brand->id }})" novalidate>
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-6">
                        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nombre de la marca</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $brand->name) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Recordatorio importante">
                        @error('name')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="logo" name="logo" value="{{ old('logo', $brand->logo) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Recordatorio importante">
                        @error('logo')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="country_origin" name="country_origin" value="{{ old('country_origin', $brand->country_origin) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Recordatorio importante">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="industry" name="industry" value="{{ old('industry', $brand->industry) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Recordatorio importante">
                        @error('industry')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        function confirmarActualizacion(event, id) {
            ...
            Swal.fire({
                ...
                text: "Se actualizará la información de esta marca.",
                icon: 'question',
