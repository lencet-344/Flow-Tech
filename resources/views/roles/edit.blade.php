    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700 p-8">
                ...
                <form action="{{ route('roles.update', $role) }}" method="POST" id="form-edit-{{ $role->id }}" onsubmit="confirmarActualizacion(event, {{ $role->id }})" novalidate>
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-6">
                        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nombre del rol</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Administrador">
                        @error('name')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="type" name="type" value="{{ old('type', $role->type) }}" maxlength="100" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors" placeholder="Ej. Administrador">
                        @error('type')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="user_id" name="user_id" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring focus:ring-indigo-500 focus:ring-opacity-20 shadow-sm transition-colors">
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $role->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script>
        function confirmarActualizacion(event, id) {
            ...
            Swal.fire({
                ...
                text: "Se actualizará la información de este rol.",
                icon: 'question',
