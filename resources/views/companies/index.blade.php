<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('companies.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150 shadow-sm">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50/50 dark:bg-gray-700/50 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 font-semibold tracking-wider">Dirección</th>
                            ...
                            @forelse ($companies as $company)
                                <tr class="group hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors bg-white dark:bg-gray-800">
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Prod: {{ $company->type_product }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $company->email }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Tel: {{ $company->telephone }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex justify-end items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ route('companies.show', $company) }}" class="text-blue-500 hover:text-blue-700 transition-colors" title="Ver">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </a>
                                            <a href="{{ route('companies.edit', $company) }}" class="text-amber-500 hover:text-amber-700 transition-colors" title="Editar">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            <form id="form-delete-{{ $company->id }}" action="{{ route('companies.destroy', $company) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                            <button type="button" onclick="confirmarEliminacion({{ $company->id }})" class="text-red-500 hover:text-red-700 transition-colors" title="Eliminar">
