...
<x-app-layout>
    <div class="p-8 md:p-10 max-w-7xl mx-auto">
        <!-- Botón de Escape -->
        <div class="mb-6">
            <a href="{{ url('/') }}" class="inline-flex items-center text-[#3b82f6] font-medium hover:underline text-sm gap-2">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 bg-gray-100 rounded-lg shrink-0">
                                        <img src="{{ $favorite->product->img ?? 'https://via.placeholder.com/100' }}" class="w-full h-full object-cover rounded-lg">
                                    </div>
                                    <div>
                                        <h4 class="text-sm font-bold text-[#040116]">{{ $favorite->product->nombre ?? 'Producto no encontrado' }}</h4>
                                        <p class="text-[13px] text-gray-500">{{ $favorite->product->proveedor->nombre ?? 'Proveedor' }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <form action="{{ route('favorites.destroy', $favorite->id ?? 0) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-50 text-red-600 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">Eliminar</button>
                                    </form>
...
<x-app-layout>
    <div class="p-8 md:p-10 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <a href="{{ route('products.index', ['category_id' => $category->id]) }}" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:-translate-y-1 hover:shadow-md transition-all">
                ...
                <div>
                    <h3 class="font-bold text-[#040116]">{{ $category->nombre ?? 'Sin nombre' }}</h3>
    ...
    <div class="p-8 md:p-10 max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-[#040116] tracking-tight">
                {{ isset($search) && $search ? 'Resultados para: "' . $search . '"' : 'Todos los productos' }}
            </h1>
            <p class="text-gray-500 text-sm mt-1">Encuentra lo que necesitas de proveedores verificados</p>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 bg-gray-100 rounded-lg shrink-0">
                                        <img src="{{ $product->img ?? 'https://via.placeholder.com/100' }}" class="w-full h-full object-cover rounded-lg">
                                    </div>
                                    <div>
                                        <h4 class="text-sm font-bold text-[#040116]">{{ $product->nombre ?? 'Sin nombre' }}</h4>
                                        <p class="text-[13px] text-gray-500">{{ $product->proveedor->nombre ?? 'Proveedor' }}</p>
                                    </div>
                                </div>
                            </td>
...
echo "Frontend views replaced successfully.";
?>
